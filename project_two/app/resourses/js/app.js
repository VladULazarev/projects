/**
 ...
 18. Slide panel
 19. Disable 'press enter' event for popup windows
 20. Get orders data
*/
...
$(".account-link, .current-user").on("tap",function(event){
...
// 12.2 Click button 'Update' for the current '.info-block' and update the info
        ...
        let updateAccountInfo = 'update-account-info';
...
$(document).on('keydown', 'html', function(e) {
    ...
    if (e.keyCode == 13 && $(".btn, .btn-danger, .btn-ok, .user-was-registered").is(":visible")){
...
// Click 'show-order-data'
$(".show-order-data").on('click', function() {

		// Hide previous results
    $('.orders-results').fadeTo(middleTime, 0);
		$('.orders-results').empty();

		// Set $_POST identifier
		let showOrderData = $(this).data("value");

		$.post("/app/controllers/OrdersController.php", {
			...
			if (data == false) {
				...
				setTimeout(function() {	$('.orders-results').html('<h4>Ничего нет</h4>')}, middleTime);

				$('.orders-results').fadeTo(middleTime, 1);
